...
export const isRefreshStep = (step: WorkflowStep) => step.type === "REFRESH";
...
const buildBatmanRefreshTicks = (
  ...
  for (const step of manualSteps) {
    if (step.type !== "TOGGLE" || step.targetEntityId !== peer.id) {
      continue;
    }
...
const buildRefreshStepsForPeer = (
  ...
  if (!peer.protocols.includes("BATMAN")) {
    return [];
  }
  ...
  return buildBatmanRefreshTicks(manualSteps, peer, maxTick).map(({ tick, startTick }) => ({
    ...
    refreshProtocol: "BATMAN" as const,
...
const normalizeManualStep = (step: WorkflowStep, index: number): WorkflowStep => {
  ...
  const safeType = step.type === "REFRESH" ? "MOVE" : step.type;
