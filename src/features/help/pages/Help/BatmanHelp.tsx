    ...
    <section className="help-page__section" id="batman">
      ...
      <ChapterBlock id="echo-location-protocol" title="Echo Location Protocol (ELP)">
        ...
        <TextBlock>
          ...
          **Neighbours List**. The node records the sequence number carried in the ELP message and
        ...
        <TableBlock
          introText="Neighbours List entry"
          ariaLabel="Neighbours List entry"
      ...
      <ChapterBlock id="throughput-calculation" title="Throughput Calculation">
        <TextBlock>
          The core function of ELP in B.A.T.M.A.N. V is the estimation of link throughput. In the
          simulator the link throughput is a static value (1000 Mbps for wired, 100 Mbps for
          wireless) that is then scaled by a **reception ratio**. The reception ratio is derived
          from the timing of received ELP packets: if the gap between the last received packet and
          the current tick is larger than the expected interval, the ratio drops below 1.0,
          reflecting packet loss. The resulting received throughput is then smoothed via EWMA.
        </TextBlock>
        <FormulaBlock formula="receptionRatio = min(1, interval / (currentTick − lastTick))" />
        <FormulaBlock formula="receivedThroughput = linkThroughput × receptionRatio" />
        <TextBlock>
      ...
      <ChapterBlock id="originator-message" title="Originator Message version 2 (OGMv2)">
        ...
        <TextBlock>
          Once validated, the node updates its **Originators Table**. Each OGMv2 carries a
        ...
        <TableBlock
          introText="Originators Table entry"
          ariaLabel="Originators Table entry"
